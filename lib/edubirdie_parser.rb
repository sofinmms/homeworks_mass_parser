...
module Parser
  class EdubirdieParser
    ...
    def self.get_next_category
      return db[:edubirdie_categories].where(is_processed: 0).first
    end
    ...
    def self.parse_products(category)
      get_pages_range(category[:url], category[:id]).each { |page_num|
        ...
        page_links = page_with_blogs.search('div.essay-list h2 a').to_a
        essays = []
        while page_links.count > 0
          ...
          pages_threads.each do |page_to_fetch|
            threads << Thread.new(page_to_fetch) do |link|
              ...
              edubirdie_products = db[:edubirdie_products].where(url: link['href']).first
        ...
        essays.each do |essay|
          db[:edubirdie_products].insert(essay)
        end
      }
      db[:edubirdie_categories].where(id: category[:id]).update(is_processed: true)
    end

    def self.get_pages_range(category_url, category_id)
      last_edubirdie_categories = db[:edubirdie_products].where(category_id: category_id).order(Sequel.desc(:page)).first
      if last_edubirdie_categories
        first_page = last_edubirdie_categories[:page].to_i
      else
        first_page = 1
      end
      agent = Mechanize.new
    ...
    def self.parse_categories
      site_categories = db[:edubirdie_categories].first
      return if site_categories

      url = 'https://edubirdie.com/examples/'
      html = URI.open(url).read
      doc = Nokogiri::HTML(html)
      doc.css('.popular-categories__list > li').each do |category|
        category_name = category.at_css('.categories-list__titling').text.strip
        link = category.at_css('.categories-list__titling')[:href]
        db[:edubirdie_categories].insert({ name: category_name, url: link })
      end
    end
    ...
    def self.get_blog(link)
      ...
      categories_elements = doc.css('div.single-content .info-table-tax-link a')
      categories = categories_elements.map(&:text)
      description = doc.css('div.single-essay')
      description.css('div.content-banner').remove
      return {
