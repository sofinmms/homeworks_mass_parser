...
module Parser
  class StudyCorgiParser
    ...
    def self.get_next_category
      return db[:study_corgi_categories].where(is_processed: 0).first
    end
    ...
    def self.parse_products(category)
      get_pages_range(category[:url], category[:id]).each { |page_num|
        ...
        page_links = page_with_blogs.search('article.archive-article a.archive-article__link').to_a
        essays = []
        while page_links.count > 0
          ...
          pages_threads.each do |page_to_fetch|
            threads << Thread.new(page_to_fetch) do |link|
              ...
              study_corgi_products = db[:study_corgi_products].where(url: link['href']).first
              unless study_corgi_products
        ...
        essays.each do |essay|
          db[:study_corgi_products].insert(essay)
        end
      }
      db[:study_corgi_categories].where(id: category[:id]).update(is_processed: true)
    end

    def self.get_pages_range(category_url, category_id)
      last_study_corgi_categories = db[:study_corgi_products].where(category_id: category_id).order(Sequel.desc(:page)).first
      if last_study_corgi_categories
        first_page = last_study_corgi_categories[:page].to_i
      else
        first_page = 1
      end
      agent = Mechanize.new
    ...
    def self.parse_categories
      site_categories = db[:study_corgi_categories].first
      return if site_categories

      url = 'https://studycorgi.com/'
      html = URI.open(url).read
      doc = Nokogiri::HTML(html)
      doc.css('.topic-grid a.topic-grid__category-link').each do |category|
        category_name = category.text.strip
        link = category[:href]
        db[:study_corgi_categories].insert({ name: category_name, url: link })
      end
    end
    ...
    def self.get_blog(link)
      ...
      categories_elements = doc.css('span.article-meta__items a')
      categories = categories_elements.map(&:text)
      description = doc.css('.custom-typography.article__content')
      description.css('.d-bnr').remove
      return {
